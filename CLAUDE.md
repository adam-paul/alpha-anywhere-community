...
### Type Organization
Types have exactly one home. No duplicates, no re-exports.

| Location | What belongs there |
|----------|-------------------|
| `src/lib/types.ts` | All domain types shared across 2+ files |
...
| Utils/stores | Implementation only — import types from `types.ts` |

- If a type is used in multiple files, it goes in `types.ts`
- Utils and stores never define types that get imported elsewhere
